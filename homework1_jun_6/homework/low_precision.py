...
class Linear4Bit(torch.nn.Module):
    ...
    def _load_state_dict_pre_hook(
        ...
        if f"{prefix}weight" in state_dict:
            ...
            w_flat = weight.reshape(-1).to(torch.float32).contiguous()
            xq4, nrm = block_quantize_4bit(w_flat, group_size=self._group_size)
            self.weight_q4.copy_(xq4.to(self.weight_q4.device))
            self.weight_norm.copy_(nrm.to(self.weight_norm.device))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Dequantize weights without tracking grads
        with torch.no_grad():
            #Dequantize the weights
            w = block_dequantize_4bit(self.weight_q4, self.weight_norm).view(self._shape)
        # Stop gradient through inputs (weights are frozen already). Bias still trains.
        return torch.nn.functional.linear(x.detach(), w, self.bias)
