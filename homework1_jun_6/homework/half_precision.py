...
class HalfLinear(torch.nn.Linear):
    def __init__(
        ...
        # Store parameters in half precision, but keep API identical to torch.nn.Linear.
        self.weight.data = self.weight.data.to(dtype=torch.float16)
        if self.bias is not None:
            self.bias.data = self.bias.data.to(dtype=torch.float16)

        # Freeze base weights – we only fine-tune LoRA adapters later on.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_dtype = x.dtype
        x_half = x.to(dtype=self.weight.dtype)
        out_half = super().forward(x_half)
        return out_half.to(dtype=x_dtype)
